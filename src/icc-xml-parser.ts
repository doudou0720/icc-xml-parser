...
export class ICCXmlParser {
  ...
  private static parseStylusPoint(pointElement: Element): StylusPoint {
    const x = parseFloat(pointElement.getAttribute("X") || "0");
    const y = parseFloat(pointElement.getAttribute("Y") || "0");
    const pressureFactor = parseFloat(
      pointElement.getAttribute("PressureFactor") || "0.5",
    );

    // Validate coordinates are valid numbers
    const validatedX = isNaN(x) ? 0 : x;
    const validatedY = isNaN(y) ? 0 : y;

    // Validate pressureFactor is within 0.0-1.0 range (per spec)
    const validatedPressureFactor = isNaN(pressureFactor)
      ? 0.5
      : Math.max(0.0, Math.min(1.0, pressureFactor));

    return {
      x: validatedX,
      y: validatedY,
      pressureFactor: validatedPressureFactor,
    };
  }
...
`;
  ...
  private static serializeDrawingAttributes(
    ...
    return `Color=${attributes.color};Width=${attributes.width};Height=${attributes.height};FitToCurve=${attributes.fitToCurve};IsHighlighter=${attributes.isHighlighter};IgnorePressure=${attributes.ignorePressure};StylusTip=${attributes.stylusTip};`;
  }
